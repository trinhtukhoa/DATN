...
//			String a=(String) model.getAttribute("count" + i);
	...
	public String AddToCart(@PathVariable int id, Model model, HttpServletRequest request) throws Exception {
		String referer = request.getHeader("Referer"); // lấy url của trang trước đó(refer) rồi điều hướng về trang trước
		User user = (User) session.getAttribute("acc"); // lấy đối tượng nd từ session, nếu k đn về null
		
		if (user == null) {
			session.setAttribute("AddToCartErr", "Vui lòng đăng nhập trước khi thực hiện thao tác");
			return "redirect:" + referer;
		} else {
			List<Cart> listCart = cartService.GetAllCartByUser_id(user.getId()); // lấy giỏ hàng của người dùng từ csdl
			Product product = productService.getProductById(id); //lấp sp theo id
			int cart = 0; // khởi tạo cart để đếm số lần add vào giỏ
			for (Cart y : listCart) {
