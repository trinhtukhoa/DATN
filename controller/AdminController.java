...
public class AdminController {
	...
	public String SignInAdminHandel(@ModelAttribute("login-name") String login_name,
	                                 @ModelAttribute("pass") String pass, Model model, HttpSession session) throws Exception {
	    ...
	    if (admin == null) {
	        ...
	        if (!decodedValue.equals(pass)) {
	            ...
	            session.setAttribute("adminName", admin.getUser_Name());  // Lưu tên admin vào session
	...
	public String DashboardOrderView(Model model, HttpSession session) {
	    User admin = (User) session.getAttribute("admin");
	    if (admin == null) {
	        return "redirect:/signin-admin";
	    } else {
	        ...
	        model.addAttribute("adminName", admin.getUser_Name());  // Truyền tên admin vào model
	...
	public String DashboardWalletView(Model model) {
	    ...
	    if (admin == null) {
	        ...
	        List<Order> listOrder = orderService.findAll();
	        // Removed momo payment method logic
	        ...
	        // List for recent orders with delivery payment method
	        ...
	        model.addAttribute("adminName", admin.getUser_Name());  // Pass admin's name to model
	...
	public String DashboardMyProductView(Model model, HttpSession session) {
	    User admin = (User) session.getAttribute("admin");
	    if (admin == null) {
	        return "redirect:/signin-admin";  // Nếu chưa đăng nhập, chuyển hướng về trang login
	    } else {
	        // Truyền danh sách categories và products vào model
	        ...
	        // Truyền thông tin admin vào model thay vì lưu vào session
	        model.addAttribute("admin", admin);
	        model.addAttribute("adminName", admin.getUser_Name());  // Pass admin's name to model
	        return "dashboard-myproducts";  // Trả về trang dashboard-myproducts
	    }
	}
...
//			int category_selected = 0;
	...
	public String DashboardMyProfile(Model model, HttpSession session) {
	    User admin = (User) session.getAttribute("admin");
	    if (admin == null) {
	        return "redirect:/signin-admin";
	    } else {
	...
	@PostMapping("/dashboard-myproducts/delete/{id}")
	public String DashboardMyProductDeleteHandle(@PathVariable int id, HttpSession session) {
	    User admin = (User) session.getAttribute("admin");
	    if (admin == null) {
	        return "redirect:/signin-admin";
	    }
